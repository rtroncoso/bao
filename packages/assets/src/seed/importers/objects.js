...
export const importObjects = ({ datsDir, debug }) => {
  ...
  for (const [sectionName, fields] of Object.entries(sections)) {
    ...
    for (const [fieldName, rawValue] of Object.entries(fields)) {
      ...
      const attributeName = FIELD_TO_ATTRIBUTE[fieldName];
      if (!attributeName) {
        if (debug) {
          console.warn(`[objects] Unmapped field ${fieldName} on OBJ${objectId}`);
        }
        continue;
      }
    ...
    statements.push(`DELETE FROM \`objects_classes\` WHERE \`objectId\`=${objectId};`);
