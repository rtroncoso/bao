...
const executeInBatches = async (
  ...
  statements: ReturnType<typeof parseSeedStatements>,
  debug: boolean,
  fileName: string,
  { respectNpcBlocks = false }: { respectNpcBlocks?: boolean } = {}
) => {
  let pending = 0;
  ...
  for (let index = 0; index < statements.length; index += 1) {
    ...
    try {
      ...
      const npcHint =
        statement.npcId !== null && statement.npcId !== undefined
          ? ` (npcId ${statement.npcId}, statement ${index + 1}/${
              statements.length
            })`
          : ` (statement ${index + 1}/${statements.length})`;
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`${message}${npcHint}`);
    }
    ...
    if (
      ...
      if (debug) {
        console.log(
          `[seed:apply] ${fileName}: committed ${index + 1}/${
            statements.length
          } statements`
        );
      }
...
export const applySeedFiles = async ({
  outputDir,
  debug,
}: {
  outputDir: string;
