...
const CRAFTING_FILES = [
  {
    fileName: "ArmasHerrero.dat",
    profession: "blacksmith_weapons",
    sectionPrefix: "Arma",
  },
  {
    fileName: "ArmadurasHerrero.dat",
    profession: "blacksmith_armor",
    sectionPrefix: "Armadura",
  },
  {
    fileName: "ObjCarpintero.dat",
    profession: "carpenter",
    sectionPrefix: "OBJ",
  },
];

export const importCrafting = ({
  datsDir,
  ...
  for (const { fileName, profession, sectionPrefix } of CRAFTING_FILES) {
    ...
    for (const [sectionName, fields] of Object.entries(sections)) {
      ...
      const objectId = Number.parseInt(
        fields.Index ?? fields.ObjIndex ?? "0",
        10
      );
