...
const parseMotdColor = (line: string) => {
...
export const importConfig = ({
      ...
      statements.push(
        upsertSql(
          "motd_lines",
          {
            lineOrder,
            text,
            colorData: parseMotdColor(String(value)),
          },
          ["lineOrder"]
        )
      );
  ...
  if (fs.existsSync(forbiddenPath)) {
    ...
    for (const rawLine of lines) {
      ...
      statements.push(
        `INSERT INTO \`forbidden_names\` (\`name\`) VALUES (${sqlValue(
          name
  ...
  if (fs.existsSync(invokarPath)) {
    ...
    const list = (sections.LIST ?? sections.List ?? sections) as Record<
      string,
      string
    >;
    let slot = 1;
    ...
    while (list[`NI${slot}`]) {
      const npcId = Number.parseInt(String(list[`NI${slot}`]), 10);
