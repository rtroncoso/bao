...
const readMetaFiles = (mapsMetaDir: string) => {
  ...
  return fs
    ...
    .map((fileName) => {
      ...
      const mapId =
        meta.mapId ?? Number.parseInt(fileName.replace(".meta.json", ""), 10);
      return { mapId, meta };
    })
...
export const importMaps = ({
  ...
  for (const { mapId, meta } of metaFiles) {
    ...
    for (const objectSpawn of meta.objects ?? []) {
      statements.push(
        `INSERT INTO \`map_object_spawns\` (\`mapId\`, \`objectId\`, \`amount\`, \`x\`, \`y\`) VALUES (${mapId}, ${
          objectSpawn.objectId
        }, ${objectSpawn.amount ?? 1}, ${objectSpawn.x}, ${objectSpawn.y});`
      );
    }
    ...
    for (
      let index = 0;
      index < blockedTiles.length;
      index += blockedBatchSize
    ) {
      const batch = blockedTiles.slice(index, index + blockedBatchSize);
      const values = batch
        .map(
          (tile: { x: number; y: number }) => `(${mapId}, ${tile.x}, ${tile.y})`
        )
        .join(", ");
      statements.push(
