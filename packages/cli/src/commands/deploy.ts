...
const resolveS3Env = (name: string) =>
  envString(`BAO_S3_${name}`) ?? envString(`AWS_S3_${name}`);

const s3KeyToInvalidationPath = (key: string) => `/${key}`;

const buildInvalidationPaths = (keys: string[]) => {
...
const loadDeployEnv = (environment: string) => {
  const repoRoot = loadRootEnv(CLI_ROOT);

  const overlayPaths = [
    path.join(CLI_ROOT, `.env.${environment}`),
...
const toPosixKey = (filePath: string, rootDir: string) =>
  path.relative(rootDir, filePath).split(path.sep).join("/");

const listRemoteObjects = async (client: S3Client, bucket: string) => {
...
const cacheControlForKey = (key: string) => {
  if (key.endsWith(".json")) {
...
export const runDeploy = async (options: DeployOptions = {}) => {
  const debug = options.debug ?? false;
  const syncAll = options.syncAll ?? false;
  const environment = options.environment ?? "staging";
  const publicDir = options.publicDir ?? resolveProjectPaths().publicDir;
  const envPath = loadDeployEnv(environment);

  if (debug) {
    console.log(`[deploy] env: ${envPath}`);
    console.log(`[deploy] public: ${publicDir}`);
  }
  ...
  const localFiles = walkFiles(publicDir);
  const localKeys = new Set(
    localFiles.map((filePath) => toPosixKey(filePath, publicDir))
  );
  ...
  for (const filePath of localFiles) {
    const key = toPosixKey(filePath, publicDir);
    const shouldUpload =
      syncAll ||
  ...
  if (!distributionId) {
    ...
    console.log(
      "[deploy] skipping CloudFront invalidation (no changed objects)"
    );
  } else {
