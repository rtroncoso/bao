...
export const selectErrors = (state: State, url: string, method: string = 'GET') => {
  const errors = getErrors(state);

  return Object.entries(errors)
    .reduce<ResponseBody | null>((acc, entry) => {
...
export const selectIsLoading = (state: State, url: string, method: string = 'GET') => {
  const queries = getQueries(state);
  return Object.entries(queries)
    .reduce<boolean>((acc, entry) => {
      const [key, value] = entry;
      const { url: matchedUrl } = JSON.parse(key);
      if (
        url === matchedUrl &&
