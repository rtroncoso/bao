...
import { GameConnectedProps } from './Game.container';
import { useKeyPress } from './Game.hooks';

export type GameProps = GameConnectedProps;
...
const Game: React.FC<GameProps> = ({ token }) => {
  ...
  useEffect(() => {
    const connection = new Client(process.env.NEXT_PUBLIC_MOB_SERVER);
    ...
    <div className="Game">
      {client ? (
        <Stage width={1600} height={900}>
          {room && characters && Object.values(characters).length
            ? Object.values(characters).map((character: any) => (
                <Container
                  ...
                  y={character.y}
                >
                  <Rectangle width={100} height={100} color={0xff0000} />
                  <Text y={105} text={character.name} style={style} />
                </Container>
              ))
            : null}
        </Stage>
